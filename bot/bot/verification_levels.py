...
class VerificationLevelsClient:
    ...
    async def process(self,message:discord.Message)->None:
        ...
        if image is None:
            await message.reply("Для цього рівня верифікації завантажте зображення ігрового профілю.",mention_author=False); return
        ...
        if response.is_error:
            await message.reply("Не вдалося проаналізувати зображення. Спробуйте ще раз або зверніться до адміністратора.",mention_author=False); return
        result=response.json()
        if not result["matched"]:
            await message.reply(f"❌ Рівень **{result['level_name']}** не підтверджено. {result.get('reason') or ''}",mention_author=False); return
