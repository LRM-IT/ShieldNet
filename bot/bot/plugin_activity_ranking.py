...
class ActivityRanking:
 ...
 async def message(self,message):
  ...
  self.cooldowns[key]=now;await self.request('POST','/activity',json={'guild_id':message.guild.id,'discord_user_id':message.author.id,'kind':'message','channel_id':str(message.channel.id),'role_ids':[str(r.id)for r in message.author.roles]})
 async def voice_state(self,member,before,after):
  ...
  if before.channel is not None and(after.channel is None or before.channel.id!=after.channel.id):
   ...
   if started:
    minutes=max(0,(now-started[0]).total_seconds()/60);await self.request('POST','/activity',json={'guild_id':member.guild.id,'discord_user_id':member.id,'kind':'voice','amount':minutes,'channel_id':started[1],'role_ids':[str(r.id)for r in member.roles]})
