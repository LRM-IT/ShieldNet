...
class ActivityRanking:
 ...
 async def message(self,message):
  ...
  self.cooldowns[key]=now;result=await self.request('POST','/activity',json={'guild_id':message.guild.id,'discord_user_id':message.author.id,'kind':'message','channel_id':str(message.channel.id),'role_ids':[str(r.id)for r in message.author.roles]});await self.apply_rewards(message.author,result,message.channel)
 ...
 async def apply_rewards(self,member,result,fallback_channel=None):
  ...
  if result.get('leveled_up')and result.get('announce_level_up'):
   channel_id=result.get('announcement_channel_id');channel=member.guild.get_channel(int(channel_id))if channel_id else fallback_channel
