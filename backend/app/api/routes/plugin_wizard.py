...
router = APIRouter(tags=["Wizard plugin"])
...
async def installation(db: AsyncSession, guild_id: int):
    return await db.scalar(select(GuildPluginInstallation).where(
        GuildPluginInstallation.guild_id == guild_id,
        GuildPluginInstallation.plugin_key == "wizard",
    ))
...
@router.get("/discord/guilds/{guild_id}/plugins/wizard")
async def state(guild_id:int,user:User=Depends(get_current_user),db:AsyncSession=Depends(get_db_session)):
    ...
    guild=await db.get(Guild,guild_id); installed=await installation(db,guild_id)
    ...
    if installed and profile and channel and profile.status=="completed" and channel.status=="completed":
        result=(channel.payload or {}).get("_result",{})
        installed.configuration={**(installed.configuration or {}),"completed":True,"game":(profile.payload or {}).get("game",""),"description":(profile.payload or {}).get("description",""),"system_channel_id":result.get("channel_id"),"system_channel_name":(channel.payload or {}).get("name")}
        await db.commit()
    return {"installed":bool(installed),"enabled":bool(installed and installed.enabled),"completed":bool(installed and (installed.configuration or {}).get("completed")),"guild":{"name":guild.name,"icon_url":guild.icon_url},"permission_check":change_payload(check),"profile_job":change_payload(profile),"channel_job":change_payload(channel),"configuration":installed.configuration if installed else {}}

@router.post("/discord/guilds/{guild_id}/plugins/wizard/check")
async def check(guild_id:int,user:User=Depends(get_current_user),db:AsyncSession=Depends(get_db_session)):
    await require_guild_management(db,user,guild_id)
    if not await installation(db,guild_id): raise HTTPException(409,"Install Wizard first")
...
@router.post("/discord/guilds/{guild_id}/plugins/wizard/apply")
async def apply(guild_id:int,payload:WizardApply,user:User=Depends(get_current_user),db:AsyncSession=Depends(get_db_session)):
    await require_guild_management(db,user,guild_id)
    installed=await installation(db,guild_id)
    if not installed: raise HTTPException(409,"Install Wizard first")
    ...
    db.add_all([profile,channel]);installed.configuration={**(installed.configuration or {}),"completed":False};await db.commit()
