...
class TemplateBankSettings(Base):
    __tablename__="settings"; __table_args__={"schema":"media"}
...
class GameLibrary(Base):
    ...
    __table_args__=(UniqueConstraint("key",name="uq_media_game_libraries_key"),{"schema":"media"})
    id:Mapped[uuid.UUID]=mapped_column(UUID(as_uuid=True),primary_key=True,default=uuid.uuid4)
    key:Mapped[str]=mapped_column(String(120),nullable=False)
    name:Mapped[str]=mapped_column(String(180),nullable=False)
    description:Mapped[str|None]=mapped_column(Text)
    icon_path:Mapped[str|None]=mapped_column(String(1000))
...
class MediaTemplate(Base):
    ...
    game_library_id:Mapped[uuid.UUID|None]=mapped_column(UUID(as_uuid=True),ForeignKey("media.game_libraries.id",ondelete="SET NULL"))
