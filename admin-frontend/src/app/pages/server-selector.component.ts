      ...
      @for(guild of guilds(); track guild.guild_id){
        <article class="card">
          <div class="guild-head">
            ...
            <span class="state" [class.online]="isConnected(guild)" [class.stale]="!(guild.custom_bot_active&&isConnected(guild))&&guild.sync_status==='stale'"><i></i>{{statusLabel(guild)}}</span>
          </div>
          <div class="stats">
...
export class ServerSelectorComponent implements OnInit,OnDestroy {
  ...
  statusLabel(g:GuildAccess){if(g.custom_bot_active&&this.isConnected(g))return this.i18n.t('server_cards.custom_bot_online','Online on custom bot');return `${this.i18n.t('server_cards.status_'+g.bot_status,g.bot_status)} · ${this.i18n.t('server_cards.sync_'+(g.sync_status||'never'),g.sync_status||'never')}`}
