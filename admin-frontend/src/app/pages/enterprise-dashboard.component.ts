              ...
              <article class="guild-card panel">
                ...
                <div class="guild-actions">
                  @if (isConnected(guild)) {
                    ...
                    <a class="connect-bot" [href]="botInstallUrl(guild.guild_id)">{{ 'dashboard.connect_bot' | snT:'Connect bot' }}</a>
                  }
                  @if (auth.profile()?.is_superadmin) { <button class="delete-guild" type="button" (click)="removeGuild(guild)">{{'platform_servers.delete_record'|snT:'Delete permanently'}}</button> }
                </div>
              </article>
            }
          </section>
...
export class EnterpriseDashboardComponent implements OnInit, OnDestroy {
  ...
  isConnected(guild: EnterpriseDashboardOverview['guilds'][number]): boolean {
    return guild.bot_status === 'online' && Boolean(guild.last_sync_at);
  }

  botInstallUrl(guildId: string): string {
