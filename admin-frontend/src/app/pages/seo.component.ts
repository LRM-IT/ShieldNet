...
<sn-shell [title]="'seo.title'|snT:'SEO and analytics'"><main class="page">
  <header class="page-head"><div><span>{{'seo.eyebrow'|snT:'SUPERADMIN'}}</span><h2>{{'seo.title'|snT:'SEO and analytics'}}</h2><p>{{'seo.help'|snT:'Homepage metadata and analytics integrations.'}}</p></div><label class="locale">{{'seo.language'|snT:'SEO language'}}<select [ngModel]="locale()" (ngModelChange)="load($event)">@for(language of i18n.languages();track language.code){<option [value]="language.code">{{language.icon}} {{language.name}}</option>}</select></label></header>
  @if(settings();as s){
    <section class="card"><h3>{{'seo.homepage'|snT:'Homepage SEO'}} · {{locale().toUpperCase()}}</h3><div class="grid">
      <label>{{'seo.site_name'|snT:'Site name'}}<input [(ngModel)]="s.site_name"></label><label>{{'seo.canonical_url'|snT:'Canonical URL'}}<input type="url" [(ngModel)]="s.canonical_url"></label>
      <label class="wide">{{'seo.meta_title'|snT:'Title'}}<input [(ngModel)]="s.title" maxlength="200"><small>{{s.title.length}} / 200</small></label>
      <label class="wide">{{'seo.description'|snT:'Description'}}<textarea rows="3" [(ngModel)]="s.description" maxlength="500"></textarea><small>{{s.description.length}} / 500</small></label>
      <label class="wide">{{'seo.keywords'|snT:'Keywords'}}<textarea rows="3" [(ngModel)]="s.keywords" maxlength="1000"></textarea></label>
      ...
      <label>{{'seo.og_image'|snT:'Open Graph image URL'}}<input type="url" [(ngModel)]="s.og_image" placeholder="https://..."></label>
      <label class="wide">{{'seo.og_title'|snT:'Open Graph title'}}<input [(ngModel)]="s.og_title" maxlength="200"></label>
      <label class="wide">{{'seo.og_description'|snT:'Open Graph description'}}<textarea rows="3" [(ngModel)]="s.og_description" maxlength="500"></textarea></label>
    </div><div class="preview"><small>{{'seo.search_preview'|snT:'SEARCH PREVIEW'}}</small><h3>{{s.title}}</h3><a>{{s.canonical_url}}</a><p>{{s.description}}</p></div></section>
    <section class="card"><div class="section-head"><div><h3>{{'seo.analytics'|snT:'Analytics'}}</h3><p>{{'seo.analytics_help'|snT:'Enter tracking identifiers. They run only after the visitor accepts cookies.'}}</p></div><label class="toggle"><input type="checkbox" [(ngModel)]="s.analytics_enabled"><span>{{'seo.enabled'|snT:'Enabled'}}</span></label></div>
      <div class="grid">
        ...
        <label>{{'seo.yandex_metrika'|snT:'Yandex Metrica ID'}}<input [(ngModel)]="s.yandex_metrika_id" placeholder="12345678"></label>
        <label>Microsoft Clarity Project ID<input [(ngModel)]="s.clarity_project_id" placeholder="abcdefghij"></label>
      </div>
    </section>
    <footer>@if(message()){<span>{{message()}}</span>}<button (click)="save()" [disabled]="saving()">{{saving()?('seo.saving'|snT:'Saving…'):('seo.save'|snT:'Save settings')}}</button></footer>
  }
