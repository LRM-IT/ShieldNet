    ...
    @else {
      ...
      @for (group of settings.groups; track group.id) {
        <section class="panel group-panel"><details><summary><strong>{{group.name}}</strong><span class="chevron">⌄</span></summary>
          ...
          <label>Мова за замовчуванням<select [(ngModel)]="group.default_language_code"><option [ngValue]="null">Без мови за замовчуванням</option>
          ...
          <p>Ця мовна роль залишиться в усіх учасників із роллю доступу, навіть якщо вони натиснуть інший прапор.</p>
          ...
          <button (click)="publish(group)" [disabled]="busy() || !settings.enabled || !group.enabled">Публікувати пост у вибраній темі</button>
...
export class PluginFirstIntroductionComponent implements OnInit {
  ...
  addGroup(): void { const id = `group${Date.now().toString(36)}`; this.settings.groups.push({id,name:'Нова група',enabled:false,channel_id:null,access_role_id:null,role_name_mask:'{group} - {name}',language_roles:{},default_language_code:null,message_id:null}); }
  ...
  async publish(group:Group): Promise<void> {
    ...
    try { await this.persist(); const result = await firstValueFrom(this.http.post<{job_id:string}>(`${base}/publish`,{group_id:group.id}));
      ...
      for (let attempt=0;attempt<30;attempt++) { await new Promise(resolve => setTimeout(resolve,1500));
        ...
        if (status.status === 'failed') throw new Error(status.error || 'Не вдалося опублікувати повідомлення.');
        if (status.status !== 'completed') continue;
        complete = true; await this.ngOnInit(); this.success.set(`Панель групи ${group.name} опублікована в Discord.`); break;
      }
      if (!complete) throw new Error('Публікація ще триває. Оновіть сторінку пізніше.');
    } catch (error:any) { this.error.set(error?.error?.detail || error?.message || 'Не вдалося опублікувати панель.'); }
