...
<div class="picker" [class.compact]="compact"><button #trigger type="button" class="trigger" [attr.aria-label]="ariaLabel" [attr.title]="compact?current()?.name:null" (click)="toggle()"><span>{{current()?.icon}}</span>@if(!compact){<strong>{{current()?.name}} — {{value.toUpperCase()}}</strong><span [class.open]="open()">⌄</span>}</button>
...
:host{display:block;position:relative;isolation:isolate}:host:has(.menu){z-index:1000}.picker{position:relative}.trigger{width:100%;min-height:52px;display:grid;grid-template-columns:34px 1fr 20px;align-items:center;gap:.55rem;padding:.65rem .8rem;text-align:left;color:var(--text);background:var(--surface-2);border:1px solid var(--line);border-radius:11px}.trigger:hover,.trigger:focus{border-color:var(--primary)}.trigger>span:first-child{font-size:1.2rem}.trigger>span:last-child{text-align:center;transition:transform .18s}.trigger>span.open{transform:rotate(180deg)}.compact .trigger{width:42px;min-height:42px;display:grid;grid-template-columns:1fr;place-items:center;padding:.35rem;border-radius:10px}.compact .trigger>span:first-child{font-size:1.15rem;line-height:1}.menu{position:fixed;z-index:10000;display:grid;gap:.25rem;max-height:min(420px,70vh);overflow-y:auto;overscroll-behavior:contain;padding:.55rem;border:1px solid var(--line-strong);border-radius:13px;background:var(--panel);box-shadow:var(--shadow)}.option{width:100%;display:grid;grid-template-columns:34px 1fr 20px;align-items:center;gap:.55rem;padding:.7rem;text-align:left;color:var(--text);border:1px solid transparent;border-radius:9px;background:transparent}.option>span:nth-child(2){display:grid;gap:.15rem}.option small{color:var(--muted);font-size:.62rem}.option:hover,.option.selected{border-color:var(--line-strong);background:var(--primary-soft)}.option>b{color:var(--primary)}
`]})
export class LanguagePickerComponent{
 @Input() ariaLabel='Select language';@Input() compact=false;@Input({required:true}) value='en';@Input({required:true}) options:LanguageEntity[]=[];@Output() valueChange=new EventEmitter<string>();
