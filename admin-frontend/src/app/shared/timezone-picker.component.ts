...
export class TimezonePickerComponent{
  ...
  filtered(){const q=this.query.trim().toLocaleLowerCase();if(q.length<2)return this.options.slice(0,150);return this.options.filter(zone=>zone.toLocaleLowerCase().includes(q)).slice(0,150)}
