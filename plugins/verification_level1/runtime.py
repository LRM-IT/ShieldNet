...
class VerificationLevel1Plugin(BackendPlugin):
    ...
    async def startup(self) -> None:
        logger.info("Verification backend started plugin_key=%s", self.key)
    async def shutdown(self) -> None:
        logger.info("Verification backend stopped plugin_key=%s", self.key)
async def setup(context: Any) -> None:
    logger.info("Verification runtime setup guild_id=%s plugin_key=%s generation=%s", getattr(context,"guild_id",None), getattr(context,"plugin_key",None), getattr(context,"generation",None))
async def start(context: Any) -> None:
    global _process_started
    _process_started=True
    logger.info("Verification runtime started guild_id=%s plugin_key=%s", getattr(context,"guild_id",None), getattr(context,"plugin_key",None))
async def stop(context: Any) -> None:
    global _process_started
    _process_started=False
    logger.info("Verification runtime stopped guild_id=%s plugin_key=%s", getattr(context,"guild_id",None), getattr(context,"plugin_key",None))
