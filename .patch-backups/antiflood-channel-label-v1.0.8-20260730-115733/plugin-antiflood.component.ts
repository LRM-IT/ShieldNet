...
interface Rule { channel_id:number; cooldown_seconds:number; enabled:boolean; }
interface Settings {
  enabled:boolean;
  ignore_bots:boolean;
...
export class PluginAntiFloodComponent implements OnInit{
  ...
  reload(){
    ...
    this.http.get<Settings>(`${this.base}/settings`).subscribe({
      next:v=>{this.settings=v;this.ruleSearch=v.rules.map(r=>this.channelName(r.channel_id))},
      error:()=>this.error.set('Unable to load AntiFlood settings.')
    });
  }
  addRule(){this.settings.rules.push({channel_id:0,cooldown_seconds:60,enabled:true});this.ruleSearch.push('');this.openRuleIndex.set(this.settings.rules.length-1)}
  ...
  searchRule(i:number,v:string){this.ruleSearch[i]=v;this.settings.rules[i].channel_id=0;this.openRule(i)}
  ...
  selectChannel(rule:Rule,c:ChannelItem){rule.channel_id=Number(c.id);const i=this.settings.rules.indexOf(rule);this.ruleSearch[i]=c.name;this.closeSelectors()}
  channelText(rule:Rule){const i=this.settings.rules.indexOf(rule);return this.ruleSearch[i]||this.channelName(rule.channel_id)}
  channelName(id:number){return this.channels().find(c=>Number(c.id)===id)?.name||String(id||'')}
  ...
  selectedRole(r:RoleItem){return this.settings.excluded_role_ids.includes(Number(r.id))}
  toggleRole(r:RoleItem){const id=Number(r.id);this.selectedRole(r)?this.removeRole(id):this.settings.excluded_role_ids.push(id)}
  removeRole(id:number){this.settings.excluded_role_ids=this.settings.excluded_role_ids.filter(x=>x!==id)}
  roleName(id:number){return this.roles().find(r=>Number(r.id)===id)?.name||String(id)}
  ...
  selectedUser(m:MemberItem){return this.settings.excluded_user_ids.includes(Number(m.discord_user_id))}
  toggleUser(m:MemberItem){const id=Number(m.discord_user_id);this.selectedUser(m)?this.removeUser(id):this.settings.excluded_user_ids.push(id)}
  removeUser(id:number){this.settings.excluded_user_ids=this.settings.excluded_user_ids.filter(x=>x!==id)}
  userName(id:number){const m=this.members().find(x=>Number(x.discord_user_id)===id);return m?.display_name||m?.username||String(id)}

  save(){
    if(this.settings.rules.some(r=>!r.channel_id)){this.error.set('Select a channel for every rule.');return}
    const ids=this.settings.rules.map(r=>r.channel_id);if(new Set(ids).size!==ids.length){this.error.set('Each channel may be used only once.');return}
    this.saving.set(true);this.error.set('');this.success.set('');
    this.http.put<Settings>(`${this.base}/settings`,this.settings).subscribe({
      next:v=>{this.settings=v;this.saving.set(false);this.success.set('AntiFlood settings saved.')},
