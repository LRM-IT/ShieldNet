...
class GuildAIRoute(Base):
    """Server-level operation route. Plugins request only a capability."""
    __tablename__ = "routes"
    __table_args__ = (UniqueConstraint("guild_id", "capability", name="uq_ai_route_guild_capability"), {"schema": "ai"})

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    guild_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("discord.guilds.guild_id", ondelete="CASCADE"), nullable=False, index=True)
    capability: Mapped[str] = mapped_column(String(80), nullable=False)
    enabled: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default="true")
    max_total_attempts: Mapped[int] = mapped_column(Integer, nullable=False, default=6, server_default="6")
    failure_threshold: Mapped[int] = mapped_column(Integer, nullable=False, default=3, server_default="3")
    cooldown_seconds: Mapped[int] = mapped_column(Integer, nullable=False, default=120, server_default="120")
    configuration: Mapped[dict[str, Any]] = mapped_column(JSONB, nullable=False, default=dict, server_default="{}")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now(), onupdate=func.now())


class GuildAIRouteTarget(Base):
    __tablename__ = "route_targets"
    __table_args__ = (UniqueConstraint("route_id", "position", name="uq_ai_route_target_position"), {"schema": "ai"})

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    route_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("ai.routes.id", ondelete="CASCADE"), nullable=False, index=True)
    provider_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("ai.providers.id", ondelete="CASCADE"), nullable=False, index=True)
...
# Kept temporarily for compatibility with older code and migration data.
