...
export class PluginAntiFloodComponent implements OnInit{
  ...
  selectChannel(rule:Rule,c:ChannelItem){rule.channel_id=Number(c.id);const i=this.settings.rules.indexOf(rule);this.ruleSearch[i]=this.channelLabel(c);this.closeSelectors()}
  ...
  channelName(id:number){const c=this.channels().find(item=>Number(item.id)===id);return c?this.channelLabel(c):String(id||'')}
