...
async def check_message(
    ...
    rule = (
        await session.execute(
            text("""
                ...
                FROM plugin_antiflood.rules
                WHERE guild_id=:guild_id AND channel_id=:channel_id AND enabled=true
            """),
            payload.model_dump(),
        )
    ).mappings().first()
    ...
    row = (
        await session.execute(
            text("""
                ...
                ON CONFLICT(guild_id,channel_id,user_id) DO UPDATE
                SET last_message_at=CASE
                    WHEN plugin_antiflood.cooldowns.last_message_at
                         <= now() - make_interval(secs=>:cooldown)
                    THEN now()
                    ELSE plugin_antiflood.cooldowns.last_message_at
                END
                RETURNING
                    last_message_at,
                    GREATEST(
    ...
    if remaining > 0:
        return {"action": "delete", "remaining": remaining, "cooldown": cooldown}
