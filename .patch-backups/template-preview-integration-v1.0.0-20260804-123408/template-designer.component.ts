        ...
        <div class="properties">
          ...
          <label>Variable
            <select [(ngModel)]="layer.variable" (change)="snapshot()">
              ...
              @for(v of variables;track v){<option [value]="v">{{v}}</option>}
            </select>
          </label>
...
export class TemplateDesignerComponent implements OnInit {
 ...
 variables=[
  '{{TITLE}}','{{DESCRIPTION}}','{{TOTAL_VOTES}}','{{CLOSED_AT}}',
  '{{WINNER_LABEL}}','{{WINNER_VOTES}}','{{WINNER_PERCENTAGE}}',
  '{{OPTION_POSITION}}','{{OPTION_LABEL}}','{{OPTION_VOTES}}','{{OPTION_PERCENTAGE}}',
  '{{SERVER_NAME}}','{{SERVER_ICON}}','{{RANK_TITLE}}','{{RANK_PERIOD}}',
  '{{ENTRY_POSITION}}','{{ENTRY_NAME}}','{{ENTRY_VALUE}}','{{QR_URL}}','{{QR_CAPTION}}'
 ];
