...
async def jobs(session: AsyncSession = Depends(get_db_session)):
    ...
    due = list((await session.execute(
        select(VotingPoll).where(
            ...
            VotingPoll.closes_at.is_not(None),
            VotingPoll.closes_at <= now,
        )
    )).scalars())
    for poll in due:
        poll.status = "closed"
        poll.closed_at = now
        exists = (await session.execute(select(VotingPublicationJob).where(
            VotingPublicationJob.poll_id == poll.id,
            VotingPublicationJob.action == "close_result",
            VotingPublicationJob.status.in_(["pending", "processing"]),
        ))).scalar_one_or_none()
        if not exists:
...
async def complete(job_id: int, payload: CompleteIn,
    ...
    if payload.result_message_id:
        poll.result_message_id = payload.result_message_id
